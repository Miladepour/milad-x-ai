...
const formShellClass =
  "flex w-full max-w-xl flex-col gap-3 lg:max-w-md lg:shrink-0 xl:max-w-lg";
...
export default function FooterNewsletterSignup() {
  ...
  function renderFormArea() {
    if (!mounted) {
      return (
        <div className={formShellClass} aria-hidden>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
            <div className="min-h-[46px] min-w-0 flex-1 rounded-sm border border-white/15 bg-white/[0.06]" />
            <div className="min-h-[46px] w-full shrink-0 rounded-sm bg-orange/40 sm:w-[140px]" />
          </div>
          {turnstileRequired ? <div className="min-h-[65px]" /> : null}
        </div>
      );
    }
